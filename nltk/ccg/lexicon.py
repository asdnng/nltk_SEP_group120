...
def matchBrackets(string):
    ...
    while rest != "" and not rest.startswith(")"):
        branch_coverage["branch_while"] = True  
        if rest.startswith("("):
            branch_coverage["branch_if_startswith_paren"] = True  
            (part, rest) = matchBrackets(rest)
            inside = inside + part
        else:
            branch_coverage["branch_else_startswith_paren"] = True  
            inside = inside + rest[0]
            rest = rest[1:]
    if rest.startswith(")"):
        branch_coverage["branch_if_startswith_close_paren"] = True
        return (inside + ")", rest[1:])
    branch_coverage["branch_raise"] = True  
